...
struct CoordT
{
  ...
  CoordT(int32_t x, int32_t y, int32_t z)
  : x(x), y(y), z(z)
  {}
...
public:
     ...
     * present.
     */
    [[nodiscard]] LeafGrid * getLeafGrid(
      const CoordT & coord,
...
template<typename DataT>
inline VoxelGrid<DataT>::VoxelGrid(
  double voxel_size,
  uint8_t inner_bits,
  uint8_t leaf_bits)
...
template<typename DataT>
inline bool VoxelGrid<DataT>::Accessor::setValue(
  const CoordT & coord,
  const DataT & value)
{
...
//----------------------------------
template<typename DataT>
inline DataT * VoxelGrid<DataT>::Accessor::value(
  const CoordT & coord,
...
//----------------------------------
template<typename DataT>
inline bool VoxelGrid<DataT>::Accessor::setCellOn(
  const CoordT & coord,
